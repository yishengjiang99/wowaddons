...
function GridColumns.ResultsValueColumn(options)
    ...
    function self.GetItemPrice(item, data)
        ...
        return core.PriceSourceHelper.GetItemPrice(item.Id == core.PriceSourceHelper.PetCageItemId and 'p:'.. item.PetId or item.Id, self.PriceSource) or item.PetId and 0
    end
