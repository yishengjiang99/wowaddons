...
local function WOWB_GetFileAgeInfos(path)
	path = path or ".";
	path = string.gsub(path, "/", "\\");
	local cmd = "dir /B /tw /a-d-h-s /od \"" .. path .. "\\\" > tmp/dir.txt"
	os.execute(cmd);
	local ret = {};
...
function dofile(filename)
	local prevCWD = _CWD;
	local str;
	str,_CWD = WOWB_ProcessFilename(filename, true);
	if(DP) then DP("FILESYS",1, "Executing \""..str.."\""); end
	WOWB_ORIG_dofile(str);
	_CWD = prevCWD;
end
