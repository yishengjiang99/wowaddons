...
function WOWB_ParseXML_InheritObj(xfpc, o, strInheritName, bVirtual)
	...
	if(not _QUICK) then
		...
		WOWB_ParseXML_InheritObj_Fly(xfpc:Recurse(oInherit), o, oInherit, o[0].name, bVirtual, {}, o[0].parentobj);
	end
	DP("XML",3 , "End");
...
function WOWB_ParseXML_FixObjectName(xfpc, o, strLastObjName)
	...
	if(WOWB_AllObjectsByName[string.lower(o[0].name)]) then
		...
		if((o[0].noglobal or "false")=="false" and (o[0].virtual or "false")=="false") then
...
function WOWB_ParseXML_QueueScripts(oCollection)
	...
	for i=1,getn(oCollection[0]) do
		...
		if((o[0].virtual or "false")=="false") then
			-- depth first; we want children to execute before parents
			if(o[0].Frames) then
...
function WOWB_ParseXML_dofile(xfpc, orgfilename)
	...
	if(not filename) then
		print("ERROR: "..xfpc.filename..":"..xfpc.linenum..": Cannot locate \""..orgfilename.."\". Skipping.");
		return;
	end
	dofile(filename);
...
function WOWB_ParseXML(orgfilename)
	...
	if(not _QUICK and not _FORCEREPARSE and WOWB_IsFileNewerThan(_CWD, orgfilename..".lua", orgfilename)) then
