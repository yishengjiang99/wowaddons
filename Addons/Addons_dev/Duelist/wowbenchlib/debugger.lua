	...
	if(type(var)=="table") then
		if(table.getn(parents)>=1 and WOWB_Debug_ClassNamesByRef and WOWB_Debug_ClassNamesByRef[var]) then
			...
			elseif(table.getn(parents)>=maxdepth) then
				...
				tinsert(aRet," {\n");
				local maxout = min((100 + 5^maxdepth)*3, 1000000); -- ~250k lines max
				for k,v in var do
					...
					if(type(v)=="function" and WOWB_Debug_ClassMethodNamesByRef and WOWB_Debug_ClassMethodNamesByRef[v] and
						...
						tinsert(aRet, " = ");
						if(_DUMPDONTFOLLOW[tostring(k)]) then
...
----------------------------------------------------------------------
-- local function offsetstackpos(n)
--
...
local function describestackpos(n)
	...
	elseif(string.sub(src,1,1)=="@") then
		...
		if(WOWB_Debug_GlobalFunctionNamesByRef[tab.func] and WOWB_Debug_GlobalFunctionNamesByRef[tab.func]~=tab.name) then
			tab.name = WOWB_Debug_GlobalFunctionNamesByRef[tab.func] .. " (via "..tab.namewhat.." "..tab.name..")";
			tab.namewhat = "global";
		end
	else
...
local function getlocalvarext(stackdepth, varname)
  local n=offsetstackpos(stackdepth);
  if(not n) then 
...
local function WOWB_ErrHandler_FormatMessage(msg,bEditDump)
	...
	for n=1,30 do
		...
		if(errdump) then
			...
			local nActual=offsetstackpos(n);
			for i=1,100 do
...
local function debugger()
	...
	if(not WOWB_Debug_CommandLine("\ndebug> ", WOWB_DebugCmds)) then
		-- willingly NOT restoring WOWB_Debugger_Recurse; it'll cause immediate return next time this is called
...
function WOWB_Debugger()
	...
	while(true) do
		local success;
		success,retval = xpcall(debugger, function(a) return a; end);
		if(not success) then
			print("BUG IN DEBUGGER: "..retval);
		else
			break; -- retval will now be true or false from debugger()
		end
	end
	WOWB_Debugger_Recurse = WOWB_Debugger_Recurse - 1;
	return b;
...
function DP(cat, lvl, msg)
	if(WOWB_DEBUGCAT[cat]>=lvl) then
		local ind = string.rep(" ", (lvl-1)*2);
		local tab = debug.getinfo(offsetstackpos(1));
